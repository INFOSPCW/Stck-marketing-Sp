...
class HelpdeskTicketMerge(models.TransientModel):
    ...
    dst_helpdesk_team_id = fields.Many2one("helpdesk.ticket.team", string="Destination Helpdesk Team")
    dst_ticket_id = fields.Many2one("helpdesk.ticket", string="Merge into an existing ticket")
    ...
    def merge_tickets(self):
        ...
        values = {
            ...
            "attachment_ids": [Command.link(a_id) for a_id in attachment_ids],
            "user_ids": [Command.link(u_id) for u_id in user_ids],
        ...
        if self.create_new_ticket:
            ...
            values.update({
                ...
                "partner_id": len(set(partner_ids)) == 1 and partner_ids[0].id or False,
                "partner_name": len(set(partner_ids)) == 1 and partner_ids[0].name or False,
                "partner_email": len(set(partner_ids)) == 1 and partner_ids[0].email or False,
                "category_id": len(set(ticket_category_ids)) == 1 and ticket_category_ids[0] or False,
        ...
        for ticket in merged_tickets:
            self._add_message(self.env._("to"), self.dst_ticket_id._get_html_link(self.dst_ticket_id.number), ticket)
        ticket_numbers = ", ".join(ticket._get_html_link(ticket.number) for ticket in merged_tickets)
        self._add_message(self.env._("from"), ticket_numbers, self.dst_ticket_id)
        merged_tickets.message_ids.filtered(lambda r: r.message_type != "notification").write(
    ...
    def _merge_description(self, tickets):
        return "\n".join(
            tickets.mapped(
                lambda ticket: self.env._("Description from ticket %(name)s: %(description)s")
                % {"name": ticket.name, "description": ticket.description or self.env._("No description")}
            )
        )
