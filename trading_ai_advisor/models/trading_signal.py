...
class TradingSignal(models.Model):
    _name        = 'trading.signal'
    _description = 'Forex Trading Signal'
    _order       = 'create_date desc'
    _inherit     = ['mail.thread', 'mail.activity.mixin']

    # ── Identity ──────────────────────────────────────────────────────────────
    name = fields.Char(string='Reference', compute='_compute_name', store=True)
    pair = fields.Selection(
        FOREX_PAIRS,
        string='Forex Pair',
        required=True,
        default='EUR/USD',
    )
    brain_id    = fields.Many2one('trading.brain', string='Brain Used',
                                  ondelete='set null')
    ...
    # ── Price data ────────────────────────────────────────────────────────────
    current_price = fields.Float(string='Price at Signal', digits=(12, 5))
    entry_price   = fields.Float(string='Suggested Entry', digits=(12, 5))
    stop_loss     = fields.Float(string='Stop Loss',       digits=(12, 5))
    take_profit   = fields.Float(string='Take Profit',     digits=(12, 5))
    risk_reward   = fields.Float(
        string='Risk/Reward',
        compute='_compute_risk_reward',
        store=True,
    )

    # ── Indicators ────────────────────────────────────────────────────────────
    rsi          = fields.Float(string='RSI (14)',     digits=(6, 2))
    macd         = fields.Float(string='MACD',         digits=(12, 6))
    ema_20       = fields.Float(string='EMA 20',       digits=(12, 5))
    ema_50       = fields.Float(string='EMA 50',       digits=(12, 5))
    ema_200      = fields.Float(string='EMA 200',      digits=(12, 5))
    ...
    def _compute_name(self):
        for rec in self:
            ...
            pair = rec.pair or 'N/A'
            rec.name = f"{pair} {rec.signal} — {ts}"
