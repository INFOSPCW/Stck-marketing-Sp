...
IMPORTANT: every create() call is wrapped in env.cr.savepoint() so that
a single bad row rolls back only itself and leaves the transaction alive.
Without savepoints a failed INSERT aborts the whole PostgreSQL transaction
and every subsequent query fails with InFailedSqlTransaction.
"""
import os
import json
import logging
...
def _load_seed_data(env):
    ...
    if data:
        _logger.info("Seed: loading %d instruments…", len(data))
...
# ─────────────────────────────────────────────────────────────────────────────
# Dataset loaders — every create() is inside env.cr.savepoint() so a bad
# row rolls back only itself and leaves the transaction intact.
# ─────────────────────────────────────────────────────────────────────────────
            ...
            for book_data in lib_data.get('books', []):
                ...
                if not exists:
                    try:
                        ...
                        _logger.warning("Seed book skipped: %s", e)
        except Exception as e:
